...
import { createQuizQuestion, deleteQuizQuestion, getQuestionByQuiz, getQuizById, updateQuizQuestion } from "../../api/Quiz";
...
class QuizQuestions extends Component {
    ...
    async onLoadQuestion() {
        const quiz = await getQuizById(this.props.match.params.quizId);
        const quest = await getQuestionByQuiz(this.props.match.params.quizId, this.props.match.params.questionNumber);
        var i;
        if(quiz.data == null){
            return this.props.history.push('/home/quiz');
        }
        ...
        this.setState({ quizTitle : quiz.data.title })
        if(quest.data != null){
            this.setState({ 
    ...
    async saveQuestion() {
        ...
        setTimeout(async() => {
            ...
            if(this.state.isQuestionExists == false){
                save = await createQuizQuestion(data);
            }else {
                save = await updateQuizQuestion(data, this.props.match.params.quizId);
            }
    ...
    async deleteQuestion() {
        swal({
            ...
            if(willDelete){
                ...
                const deleted = await deleteQuizQuestion(this.state.quizQuestionId);
    ...
    async onPage(type){
        if(type == 'next'){
            return this.props.history.push(`/home/quiz/${this.props.match.params.quizId}/${parseFloat(this.props.match.params.questionNumber) + parseFloat(1)}`);
        }else{
            this.props.history.push(`/home/quiz/${this.props.match.params.quizId}/${parseFloat(this.props.match.params.questionNumber) - parseFloat(1)}`);
        }
    }
                                        ...
                                        <div className="row ml-2">
                                            {
                                                this.state.listOfNumber.map((list, number) => {
                                                    return (
                                                        // <div class="col-3">
                                                            <Link key={number} to={`/home/quiz/${this.props.match.params.quizId}/${list}`} className={`${this.props.match.params.questionNumber == list ? 'btn btn-primary' : 'btn btn-light'} my-1 ml-1 col-3 w-100 font-weight-bold`}>{list}</Link>
                                                        // </div>
                                                    )
                                                })           
                                            }
