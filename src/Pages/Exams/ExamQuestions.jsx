...
class ExamQuestions extends Component {
    ...
    async onLoadQuestion() {
        ...
        this.setState({ examTitle : exam.data.title })
        if(quest.data != null){
            this.setState({ 
                ...
                choiceValue : quest.data.exams_multiple_choices,
                isQuestionExists : true,
                examQuestionId : quest.data.id
            });
        }else{
    ...
    async saveQuestion() {
        ...
        setTimeout(async() => {
            ...
            if(this.state.isQuestionExists == false){
                save = await createExamQuestion(data);
            }else {
                save = await updateExamQuestion(data, this.props.match.params.examId);
            }
    ...
    async deleteQuestion() {
        swal({
            ...
            if(willDelete){
                ...
                const deleted = await deleteExamQuestion(this.state.examQuestionId);
    ...
    async onPage(type){
        if(type == 'next'){
            return this.props.history.push(`/home/exams/${this.props.match.params.examId}/${parseFloat(this.props.match.params.questionNumber) + parseFloat(1)}`);
        }else{
            this.props.history.push(`/home/exams/${this.props.match.params.examId}/${parseFloat(this.props.match.params.questionNumber) - parseFloat(1)}`);
        }
    }
