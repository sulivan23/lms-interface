...
import { getResultQuizContest } from "../../../api/QuizContest";

class ResultQuizContest extends Component { 
    ...
    async getResult() {
        ...
        const result = await getResultQuizContest(param);
            ...
            <div className="main-content">
                <section className="section">
                    <div className="section-header">
                        <h1>Result Quiz Contest</h1>
                        <div className="section-header-breadcrumb">
                            <div className="breadcrumb-item active">
                                <Link to="/home">Dashboard</Link>
                            </div>
                            <div className="breadcrumb-item">Result Quiz Contest</div>
                        </div>
                    </div>
                                                ...
                                                <tbody>
                                                    {
                                                        this.state.resultData.map((result, i) => {
                                                            return (
                                                                <tr key={i}>
                                                                    <td>{result.id}</td>
                                                                    <td>{result.quiz_contest.title}</td>
                                                                    <td>{result.status}</td>
                                                                    ...
                                                                    <td>
                                                                        {
                                                                            result.status == 'Done' ?
                                                                                <Link class="btn btn-primary my-2 mb-2 w-100" to={`/home/quiz_contest_answer/${result.quiz_contest.id}/1/result`}>Detail</Link>
                                                                            :
                                                                            ''
                                                                        }
                                                                    </td>
...
export default ResultQuizContest;